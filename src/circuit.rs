...
fn ch<F: RichField + Extendable<D>, const D: usize>(
    ...
    ch_u32_by_table(builder, a, b, c, table_idx)
}
...
fn maj<F: RichField + Extendable<D>, const D: usize>(
    ...
    maj_u32_by_table(builder, a, b, c, table_idx)
}
...
pub struct Sha256U32Targets {
    pub message: Vec<U32Target>,
    pub digest: Vec<BoolTarget>,
}
...
pub fn fill_u32_circuits<F: RichField + Extendable<D>, const D: usize>(
    pw: &mut PartialWitness<F>,
    msg: &[u8],
    targets: &Sha256U32Targets,
) -> Result<()> {
    ...
    for i in 0..msg_blocks * 16 {
        let mut u32_target = 0;
        for j in 0..32 {
            let idx = i*32 + j;
            let bit = if idx < msg_bits_len {
                msg_bits[idx]
            } else if idx == msg_bits_len {
                true // the mandatory `1` bit
            } else if idx >= msg_blocks * 512 - 64 {
                // length encoding, big-endian
                ((msg_bits_len >> (msg_blocks * 512 - idx - 1)) & 1) == 1
            } else {
                false
            };
            u32_target |= (bit as u32) << (31-j);
        }
        pw.set_u32_target(targets.message[i], u32_target)?;
    }
    Ok(())
}

/// Returns (table_index, input_target, output_target)
fn build_nibble_lut<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
) -> usize {
    const LUT_SIZE: usize = (1 << 12) * 3;
    let inputs: [u16; LUT_SIZE] = core::array::from_fn(|i| i as u16);

    let lut_fn = |key_full: u16| -> u16 {
        let key = key_full & 0xfff;
        if key_full < (1 << 12) {
            let a = (key >> 8) as u16;
            let b = ((key & 0xff) >> 4) as u16;
            let c = (key & 0xf) as u16;
            (a ^ b ^ c) as u16 // xor3
        } else if key_full < (1 << 12) * 2 {
            let a = (key >> 8) as u16;
            let b = ((key & 0xff) >> 4) as u16;
            let c = (key & 0xf) as u16;
            (a & b) ^ (a & c) ^ (b & c) as u16 // maj
        } else {
            let a = (key >> 8) as u16;
            let b = ((key & 0xff) >> 4) as u16;
            let c = (key & 0xf) as u16;
    ...
    let table_index = builder.add_lookup_table_from_fn(lut_fn, &inputs);
...
fn arithmetic_u32_by_table<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: &U32Target,
    b: &U32Target,
    c: &U32Target,
    ...
    let mut nibble_results = Vec::with_capacity(8);
    for i in 0..8 {
        ...
        let key = builder.add_const(key, F::from_canonical_u64(offset));
        let nibble_result = builder.add_lookup_from_index(key, table_idx); // u16 result
        nibble_results.push(nibble_result);
    }

    let mut acc = nibble_results[0];
    let mut pow = F::from_canonical_u64(16);
    for i in 1..8 {
        let term = builder.mul_const(pow, nibble_results[i]);
...
fn xor3_u32_by_table<F: RichField + Extendable<D>, const D: usize>(
    ...
    arithmetic_u32_by_table(builder, a, b, c, table_idx, 0)
}

fn maj_u32_by_table<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: &U32Target,
    b: &U32Target,
    c: &U32Target,
    ...
    arithmetic_u32_by_table(builder, a, b, c, table_idx, 1 << 12)
}

fn ch_u32_by_table<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: &U32Target,
    b: &U32Target,
    c: &U32Target,
    ...
    arithmetic_u32_by_table(builder, a, b, c, table_idx, (1 << 12) * 2)
}
...
pub fn make_circuits<F: RichField + Extendable<D>, const D: usize>(
    ...
    let table_idx = build_nibble_lut(builder);
    ...
    for blk in 0..block_count {
        ...
        for i in 0..16 {
            ...
            let ch_e_f_g = ch(builder, &e, &f, &g, table_idx);
            ...
            let maj_a_b_c = maj(builder, &a, &b, &c, table_idx);
        ...
        for (i, z) in [a, b, c, d, e, f, g, h].iter().enumerate() {
            state[i] = add_u32(builder, &state[i], z);
        }
    }
...
pub fn make_u32_circuits<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    total_bits: usize,
) -> Sha256U32Targets {
    let mut message = Vec::new();
    ...
    assert!(total_bits % 512 == 0, "total_bits must be a multiple of 512");
    let block_count = total_bits / 512;



    for _ in 0..total_bits / 32 {
        message.push(builder.add_virtual_u32_target());
    }
    ...
    println!("u32 num_gates: {}", builder.num_gates());
    let table_idx = build_nibble_lut(builder);
    println!("u32 num_gates: {}", builder.num_gates());
    ...
    for blk in 0..block_count {
        ...
        for i in 0..16 {
            let u32_target = message[blk * 16 + i];
            x.push(u32_target);
    ...
    Sha256U32Targets { message, digest }
}
...
pub mod tests {
    ...
    use crate::circuit::{
        array_to_bits, fill_u32_circuits, make_circuits, make_u32_circuits, EXAMPLE_MESSAGE
    };

    pub const fn sha256_wide_config() -> CircuitConfig {
        CircuitConfig {
            num_wires: 176, // ≈128 advice + 48 routed
            num_routed_wires: 48,
            num_constants: 2,
    ...
    #[test]
    #[ignore]
    fn test_sha256_u32_circuit_short() -> anyhow::Result<()> {
        const D: usize = 2;
        type C = PoseidonGoldilocksConfig;
        type F = <C as GenericConfig<D>>::F;
        let mut builder = CircuitBuilder::<F, D>::new(CircuitConfig::standard_recursion_config());
        ...
        // Create SHA256 circuit using your make_circuits function
        let sha256_targets = make_u32_circuits(&mut builder, (msg_len_in_bits + 65 + 511) / 512 * 512);

        // Create witness
        let mut pw = PartialWitness::new();

        fill_u32_circuits::<F, D>(&mut pw, &msg, &sha256_targets)?;
    ...
    #[test]
    fn test_sha256_u32_circuit() -> anyhow::Result<()> {
        const D: usize = 2;
        type C = PoseidonGoldilocksConfig;
        type F = <C as GenericConfig<D>>::F;
        let mut builder = CircuitBuilder::<F, D>::new(CircuitConfig::standard_recursion_config());
        ...
        println!(
            "U32: Constructing inner proof with {} gates",
            builder.num_gates()
        );
